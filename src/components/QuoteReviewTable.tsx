...
function CommentPopover({ value, onSave, onCancel, anchorEl }: { value: string; onSave: (v: string | null) => void; onCancel: () => void; anchorEl: HTMLElement | null }) {
  ...
  return createPortal(
    <div
      ...
      style={{ top: pos.top, left: pos.left, fontFamily: "'Manrope', sans-serif" }}
    >
      <p className="text-sm font-semibold text-[#032D60] mb-2">Comentario de linea</p>
      <textarea
        value={text}
      ...
      <div className="flex items-center justify-between mt-3">
        ...
        <div className="flex gap-2">
          <button
            onClick={onCancel}
            className="px-3 py-1.5 text-xs font-semibold text-[#444444] bg-[#F0F0F0] rounded-md hover:bg-[#E5E5E5] transition-colors"
          >
            Cancelar
          </button>
          <button
            onClick={() => onSave(text.trim() || null)}
            className="px-3 py-1.5 text-xs font-semibold text-white bg-[#0176D3] rounded-md hover:bg-[#014486] transition-colors"
          >
            Guardar
          </button>
        </div>
...
export default function QuoteReviewTable({
  ...
  return (
    <div className="px-7 pb-4">
      ...
      {commentingIndex !== null && onCommentSave && (
        <CommentPopover
          value={lines[commentingIndex]?.comentario || ''}
          onSave={(v) => { onCommentSave(commentingIndex, v); closeComment(); }}
